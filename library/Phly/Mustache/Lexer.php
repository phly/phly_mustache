...
class Lexer
{
    ...
    public function compile($string)
    {
        ...
        for ($i = 0; $i < $len; ) {
            switch ($state) {
                        ...
                        switch ($tagData[0]) {
                            ...
                            case '=':
                                ...
                                if (!$preg_match('/^=(\S+)\s+(\S+)=$/', $tagData, $matches)) {
                                    throw new \Exception('Did not find delimiters!');
                                }
                            ...
                            default:
                                ...
                                if (!preg_match($this->patterns[self::VARNAME], $tagData)) {
                                    throw new \Exception('Invalid variable name provided (' . $tagData . ')');
                                }
                ...
                case self::STATE_SECTION:
                    ...
                    break;
                default:
                    throw new \Exception('Invalid state invoked ("' . var_export($state, 1) . '")?');
            }
        }
        ...
        switch ($state) {
            ...
            case self::STATE_TAG:
                // Un-closed content
                throw new \Exception('Unbalanced tag in template');
            case self::STATE_SECTION:
                // Un-closed section
                throw new \Exception('Unbalanced section in template');
        }
